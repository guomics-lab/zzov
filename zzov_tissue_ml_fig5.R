...
########################################################################
# setwd("E:/Qsync/work/zzov/ZZOV/ZZOV/experiment20220120/matrix_20221013/xgboxt/tissue_mrm")
protmat <-read.csv("20221115zzova_tissue_MRM_matrix_deleteCiRT_71prot.csv",header = T,row.names = 1)

protmat<-protmat[,!grepl("*pool",colnames(protmat))]

protmat1<-data.frame(t(protmat))
library(xlsx)
samp_inf<-read.xlsx("20221116sample_info_final_20230201.xlsx",sheetIndex = 1)

protmat1$batch<-row.names(protmat1)

protmat1$pat_id<-samp_inf$patientID[match(protmat1$batch,samp_inf$batchID)]
...
unisamp<-merge(allsample,samp_inf,by.x ="batch" ,by.y ="batchID" ,all.x = T)

#############################################################################
MRM_mat<-unisamp[,-c(73,74,75)]

# MRM_mat1$age<-MRM_mat$Age[MRM_mat$patient_barcode%in%MRM_mat1$SwissprotID]
vali_mat<-MRM_mat[grepl("vali",MRM_mat$train_test_vali),]
train_mat<-MRM_mat[!grepl("vali",MRM_mat$train_test_vali),]
#In vali_mat, several samples that are not present in dia need to be removed. 
#These samples are not required, so they should not be included in the train set either.
del_samp<-c("b50_12","b50_9","b51_11","b51_9","b52_12","b53_11","b53_7" )
vali_mat<-vali_mat[!vali_mat$batch%in%del_samp,]

#################################################################################################
#ttest
MRM_mat1<-log2(train_mat[,2:72])
mm<-train_mat[,2:72]
MRM_mat1[mm==0]<-0
...
# write.csv(FC_PAd,"20230201_zzov_tissue_mrm_0_vs_1_t_test_FC_PAd.csv")
...
pro_res <- data.frame(sapply(MRM_mat1[,1:71], res_stan))
...
# write.csv(prot_list,"20230201_zzov_tissue_mrm_input_feature.csv",row.names = F)

######################################################################################################
train_prot<-train_mat[,colnames(train_mat)%in%prot_list]
train_prot<-cbind(train_prot,train_mat[,c(73:76,80)])
row.names(train_prot)<-train_mat$batch
...
temp_mean<-apply(train_prot[,1:40],2, mean, na.rm = TRUE)
temp_sd<-apply(train_prot[,1:40],2, sd, na.rm = TRUE)
train_prot[,1:40]<-scale(train_prot[,1:40],scale = T,center = T)
...
vali_mat2<-cbind(vali_mat2,vali_mat[,c(73:76,80)])
row.names(vali_mat2)<-vali_mat$batch
...
#############################
#Del P01591；P01903
del_prot<-c("P01591","P01903")
train_prot<-train_prot[,!(names(train_prot)%in%del_prot)]
train_prot$pat_id<-samp_inf$patientID[match(row.names(train_prot),samp_inf$batchID)]
# write.csv(train_prot,"20230201_zzov_tissue_mrm_ML_train_input.csv",row.names = T)

vali_mat2<-vali_mat2[,!(names(vali_mat2)%in%del_prot)]
vali_mat2$pat_id<-samp_inf$patientID[match(row.names(vali_mat2),samp_inf$batchID)]
# write.csv(vali_mat2,"20230220_zzov_tissue_mrm_ML_vali_input.csv",row.names = T)
# sum(unique(vali_mat2$pat_id))
...
for (tim in tims) {
  ...
  train_0<-sample(mat1_pat,100)
  train_1<-sample(mat2_pat,156)
...
# write.csv(table_temp,"20230201_zzov_tissue_MRM_xdboostt_step1_table.csv",row.names = F)
library(plyr)
library(magrittr)
library(rlist)
...
impt1<-impt[,c(1,9)]
impt2<-impt[,-1]
mean_temp <- aggregate(impt2,by=list(impt2$eta_s,impt2$ss,impt2$prot,impt$tims),mean)
...
# write.csv(mean_temp,"20230201_zzov_tissue_MRM_xdboost_step1_mean.csv",row.names = F)
...
write.csv(accu3,"20230201_zzova_xgboost_tissue_MRM_step2_mean.csv",row.names = F)

#############################################################
#Further parameter selection.

mean_temp<-read.csv("20230201_zzov_tissue_MRM_xdboost_step1_mean.csv",header = T)
accu3<-read.csv("20230201_zzova_xgboost_tissue_MRM_step2_mean.csv",header = T)

accu4<-accu3[accu3$acc_test>0.7,]
a<-unique(accu4$tims)
accu4_1<-accu4[(accu4$tims==a[1]|accu4$tims==a[2]),]
accu4_2<-accu4[(accu4$tims==a[3]|accu4$tims==a[4])|(accu4$tims==a[5]|accu4$tims==a[6]),]
accu4_3<-accu4[(accu4$tims==a[7]|accu4$tims==a[8]),]

accu5<-c()

for (i in 1:nrow(accu4_3)) {
  for (gam in seq(0,0.2,0.05)) {
    for (max_dep in seq(3,10,1)) {
      for (colsamp in seq(0.1,1,0.1)) {
        for(min_chid in seq(1,5,1)) {
          
          indsel<-mean_temp[mean_temp$type==paste(accu4_3$eta_s[i],accu4_3$ss[i],accu4_3$tims[i],sep = "_"),]
          ...
          eta_s<-accu4_3$eta_s[i]
          ss<-accu4_3$ss[i]
          feature_choose<-indsel$feature[order(indsel$Frequency,decreasing=T)][1:accu4_3$fea_selt[i]]
          ...
          accu5 <- rbind(accu5,c(row.names(accu4_3)[i],accu4_3$tims[i],eta_s,ss,gam,max_dep,colsamp,min_chid,accu4_3$fea_selt[i],acc_train,auc_train,acc_test,auc_test,acc_vali,auc_vali))
        }
      }
    }
...
write.csv(accu5,"20230208_zzov_xgboost_tissue_MRM_step3_3_3.csv",row.names = F)


##########################################################
#Plotting based on the selected models.
mean_temp<-read.csv("20230201_zzov_tissue_MRM_xdboost_step1_mean.csv",header = T)
accu3<-read.csv("20230201_zzova_xgboost_tissue_MRM_step2_mean.csv",header = T)
unique(accu3$tims)
#4938
eta_s=0.26;ss=0.9;gam=0.2;max_dep=8;colsamp=0.9;min_chid=2 ;fea_selt=14

indsel<-mean_temp[mean_temp$type==paste(accu3$eta_s[4938],accu3$ss[4938],accu3$tims[4938],sep = "_"),]

set.seed(unique(accu3$tims)[7])
mat1_pat<-unique(mat1$pat_id)
mat2_pat<-unique(mat2$pat_id)
train_0<-sample(mat1_pat,100)
train_1<-sample(mat2_pat,156)
train_mat<-rbind(train_prot[train_prot$pat_id%in%train_0,-ncol(train_prot)],train_prot[train_prot$pat_id%in%train_1,-ncol(train_prot)])
test_mat<-train_prot[!row.names(train_prot)%in%row.names(train_mat),-ncol(train_prot)]
# write.csv(train_mat,"20230220_tissue_mrm_final_train_matrix.csv",row.names = T)
# write.csv(test_mat,"20230220_tissue_mrm_final_test_matrix.csv",row.names = T)

###########################
feature_choose<-indsel$feature[order(indsel$Frequency,decreasing=T)][1:fea_selt]
feature_choose<-feature_choose[-c(2,4,6)]
# write.csv(feature_choose,paste0("20230216_zzov_xgboost_tissue_MRM_feature_accu3_mode4938.csv"),row.names = F)
# feature_choose<-importance$Feature[importance$Gain>fea_selt]
train_mat2<-train_mat[,colnames(train_mat)%in%feature_choose]
train_mat2$label<-train_mat$label
# train_mat2<-train_mat_all[,colnames(train_mat_all)%in%feature_choose]
# train_mat2$label<-train_mat_all$label
##############
test2<-test_mat[,colnames(test_mat)%in%feature_choose]
test2$label<-test_mat$label
vali2<-vali_mat2[,colnames(vali_mat2)%in%feature_choose]
...
pre_xgb = round(predict(xgb2,newdata = dtrain2))
pre_xgb_train<-predict(xgb2,newdata = dtrain2)
bbb<-cbind(train_label2,pre_xgb_train)
aaa<-data.frame(table(train_label2,pre_xgb,dnn=c("true","pre")))
acc_train<-sum(pre_xgb==train_label2)/length(train_label2)
...
pdf("20240202_zzov_xgboost_tissue_MRM_train_all_feature_final_accu3_mode4938_del_3feature.pdf",width = 6,height = 6)
...
write.csv(df.score,"20240202_zzov_xgboost_tissue_MRM_train_all_feature_final_pred_accu3_mode4938_del_3feature.csv",row.names = T)
...
pdf("20240202_zzov_xgboost_tissue_MRM_train_all_feature_final_kmplot_accu3_mode4938_del_3feature.pdf", width = 8, height = 6)
ggsurvplot(tmp.cox,
...
pdf("20240202_zzov_xgboost_tissue_MRM_test_feature_final_accu3_mode4938_del_3feature.pdf",width = 8,height = 8)
shap.plot.summary(shap_values,dilute = 1)
dev.off()
...
df.score$score<-ifelse(df.score$score<0.5,"0","1")
df.score$score<-as.numeric(df.score$score)
write.csv(df.score,"20240202_zzov_xgboost_tissue_MRM_test_feature_final_pred_accu3_mode4938_del_3feature.csv",row.names = T)
df.score$cox_recurrence<-ifelse(df.score$cox_recurrence=="Yes","1","0")
df.score$cox_recurrence<-as.numeric(df.score$cox_recurrence)
tmp.cox <-  survfit(Surv(cox_RFS,cox_recurrence)~score, data=df.score)

pdf("20240202_zzov_xgboost_tissue_MRM_test_feature_final_kmplot_accu3_mode4938_del_3feature.pdf", width = 8, height = 6)
ggsurvplot(tmp.cox,
           pval = TRUE, conf.int = TRUE,
           risk.table = TRUE, # Add risk table
           risk.table.col = "strata", # Change risk table color by groups
...
pdf("20240202_zzov_xgboost_tissue_MRM_vali_feature_final_accu3_mode4938_del_3feature.pdf",width = 5,height = 5)
shap.plot.summary(shap_values,dilute = 1)
dev.off()
...
df.score$score<-ifelse(df.score$score<0.5,"0","1")
df.score$score<-as.numeric(df.score$score)
write.csv(df.score,"20240202_zzov_xgboost_tissue_MRM_vali_feature_final_pred_accu3_mode4938_del_3feature.csv",row.names = T)
df.score$cox_recurrence<-ifelse(df.score$cox_recurrence=="Yes","1","0")
df.score$cox_recurrence<-as.numeric(df.score$cox_recurrence)
tmp.cox <-  survfit(Surv(cox_RFS,cox_recurrence)~score, data=df.score)

pdf("20240202_zzov_xgboost_tissue_MRM_vali_feature_final_kmplot_accu3_mode4938_del_3feature.pdf", width = 8, height = 6)
ggsurvplot(tmp.cox,
           pval = TRUE, conf.int = TRUE,
           risk.table = TRUE, # Add risk table
           risk.table.col = "strata", # Change risk table color by groups
...
#1、Del the specified four samples.

library(openxlsx)

#Del PNNL-
tcga<-read.xlsx("20240204_zzov_TCGA_11prot_inf_126samp.xlsx",sheet=1,rowNames = T)


tcga$label<-0
tcga$label[tcga$`PlatinumFreeInterval.(mos)`>12]<-1
tcga1<-tcga[,c(1:11,17)]
tcga1<-tcga1[,c(11,9,8,1,2,5,6,7,4,3,10,12)]

tcga1[,1:11]<-scale(tcga1[,1:11],scale = T,center = T)

tcga_matrix2 <- sparse.model.matrix(label ~ ., data = tcga1)
tcga_label2 <- as.numeric(tcga1$label)
tcga_fin2 <- list(data=tcga_matrix2,label=tcga_label2) 
dtcga2 <- xgb.DMatrix(data = tcga_fin2$data, label = tcga_fin2$label) 

########################################
#tcga
#

pre_xgb_tcga = round(predict(xgb2,newdata = dtcga2))
pre_xgb_tcge1 = predict(xgb2,newdata = dtcga2)
aaa<-data.frame(table(tcga_label2,pre_xgb_tcga,dnn=c("true","pre")))
acc_test<-sum(pre_xgb_tcga==tcga_label2)/length(tcga_label2)

#ROC
library(pROC)
xgboost_roc_tcga <- roc(tcga_label2,as.numeric(pre_xgb_tcge1))

auc_tcga<-xgboost_roc_tcga[["auc"]]
...
X2<-as.matrix(tcga1[,-ncol(tcga1)])
shap_values <- shap.prep(xgb_model = xgb3, X_train = X2)

pdf("20240204_zzov_xgboost_tissue_MRM_tcga_feature_final_accu3_mode4938_del_3feature_scale_PNNL_del.pdf",width = 8,height = 8)
shap.plot.summary(shap_values,dilute = 1)
dev.off()

#kmplot
df.score<-data.frame(row.names(tcga1),as.numeric(pre_xgb_tcge1))
names(df.score)<-c("Sample_name","score")
df.score$label<-tcga$label[match(df.score$Sample_name,row.names(tcga))]
df.score$cox_recurrence<-tcga$Prog_or_Recur[match(df.score$Sample_name,row.names(tcga))]
df.score$cox_RFS<-tcga$`PlatinumFreeInterval.(mos)`[match(df.score$Sample_name,row.names(tcga))]
df.score$score<-ifelse(df.score$score<0.5,"0","1")
df.score$score<-as.numeric(df.score$score)
write.csv(df.score,"20240204_zzov_xgboost_tissue_MRM_tcga_feature_final_pred_accu3_mode4938_del_3feature_scale_PNNL_del.csv",row.names = T)
df.score$cox_recurrence<-ifelse(df.score$cox_recurrence=="Yes","1","0")
df.score$cox_recurrence<-as.numeric(df.score$cox_recurrence)
tmp.cox <-  survfit(Surv(cox_RFS,cox_recurrence)~score, data=df.score)

pdf("20240204_zzov_xgboost_tissue_MRM_tcga_feature_final_kmplot_accu3_mode4938_del_3feature_scale_PNNL_del.pdf", width = 8, height = 6)
ggsurvplot(tmp.cox,
           pval = TRUE, conf.int = TRUE,
           risk.table = TRUE, # Add risk table
           risk.table.col = "strata", # Change risk table color by groups
...
#day
df.score$day<-df.score$cox_RFS*30
tmp.cox <-  survfit(Surv(day,cox_recurrence)~score, data=df.score)

pdf("20240204_zzov_xgboost_tissue_MRM_tcga_feature_final_kmplot_accu3_mode4938_del_3feature_day_scale_PNNL_del.pdf", width = 8, height = 6)
ggsurvplot(tmp.cox,
           pval = TRUE, conf.int = TRUE,
           risk.table = TRUE, # Add risk table
           risk.table.col = "strata", # Change risk table color by groups
