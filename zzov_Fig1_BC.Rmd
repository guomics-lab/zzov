...
### 2021.11.22 color legend 改成gradient的形式
library(scales)
...
write.xlsx(df.pca, "PCA_1041.xlsx", row.names = T)
...
df = cor(pm_2samples,use = "pairwise.complete.obs")
df1 = df+0.5
...
```{r violin - FigS1 G}
# mouse liver
pm_ml <- read.delim("20210129ZZOV_QCML_liverlib_excludedFromAssay_prot_matrix.txt")  
...
#pm_pool = data.frame(log2(pm_pool[,-1]),
#                     row.names = pm_pool$prot)
...
ggplot(data_violin, aes(x, y, fill=x)) +
